...
class CMediaType : public IMediaType , public ISerialize
{
public:
    ...
    STDMETHODIMP_(uint32_t) Compare(IMediaType* pMT);
    ...
    STDMETHODIMP SetStreamInfo(int64_t* start_time,int64_t* length,int64_t* bitrate,bool* global_header,uint8_t* extra_data,int* extra_size,uint8_t** extra_data_out);
    ...
    STDMETHODIMP SetFrame(IMediaFrame* pFrame);
    STDMETHODIMP_(IMediaFrame*) GetFrame();
    //ISerialize
    ...
    static uint32_t Compare(IMediaType* pDest,IMediaType* pSour);
protected:
    MediaMajorType m_major;
    const AVCodecDescriptor* m_pDesp;
    dom_ptr<ISerialize> m_spProfile;
