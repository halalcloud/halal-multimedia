...
HRESULT CMemProfile::CValue::Load(IStream* pStream,uint8_t flag)
{
    HRESULT hr;
    uint32_t szStr  = 0;
    JIF(pStream->Read(&szStr,sizeof(szStr),flag));
    JCHK(m_val.type = (IProfile::Type)realloc((void*)m_val.type,szStr),E_OUTOFMEMORY);
    JIF(pStream->Read((void*)m_val.type,szStr,flag));
    if(IID_CMP(IID(IProfile),m_val.type))
    {
        dom_ptr<IProfile> spProfile;
...
HRESULT CMemProfile::CValue::Save(IStream* pStream,uint8_t flag)
{
    HRESULT hr;
    JCHK(NULL != pStream,E_INVALIDARG);
    uint32_t szStr  = strlen(m_val.type) + 1;
    JIF(pStream->Write(&szStr,sizeof(szStr),flag & (~IStream::WRITE_FLAG_REFFER)));
    JIF(pStream->Write((void*)m_val.type,szStr,flag));
    if(IID_CMP(IID(IProfile),m_val.type))
    {
    ...
    {
        JIF(pStream->Write(&m_val.len,sizeof(m_val.len),flag));
        JIF(pStream->Write(m_val.value,m_val.len,flag));
    }
    return hr;
}
...
STDMETHODIMP_(uint32_t) CMemProfile::Count()
{
...
STDMETHODIMP_(uint32_t) CMemProfile::Compare(IProfile* pProfile)
{
    JCHK(NULL != pProfile,COMPARE_DIFFERENT_VALUE);
    return Compare(this,pProfile);
}

STDMETHODIMP CMemProfile::Load(IStream* pStream,uint8_t flag,void* param)
{
    JCHK(NULL != pStream,E_INVALIDARG);
    HRESULT hr;
    uint32_t count = 0;
    JIF(pStream->Read(&count,sizeof(count),flag));
    while(0 < count--)
    {
        CValue* pVal;
        uint32_t szStr;
        ...
        m_values.insert(CValue::Pair(name,pVal));
        JIF(pVal->Load(pStream,flag));

    }
    return hr;
}

STDMETHODIMP CMemProfile::Save(IStream* pStream,uint8_t flag,void* param)
{
    HRESULT hr;
    JCHK(NULL != pStream,E_INVALIDARG);
    uint32_t count = m_values.size();
    JIF(pStream->Write(&count,sizeof(count),flag & (~IStream::WRITE_FLAG_REFFER)));
    for(CValue::It it = m_values.begin() ; it != m_values.end() ; ++it)
    {
        uint32_t szStr = it->first.size();
        JIF(pStream->Write(&szStr,sizeof(szStr),flag & (~IStream::WRITE_FLAG_REFFER)));
        JIF(pStream->Write((void*)it->first.data(),szStr,flag));
        JIF(it->second->Save(pStream,flag));
    }
    return hr;
}
...
uint32_t CMemProfile::Compare(IProfile* pDest,IProfile* pSour)
{
    if(pDest == pSour)
    ...
    while(NULL != (key = pDest->Next(key)))
    {
        ...
        if(IID_CMP(IID(IProfile),pValDest->type))
        {
            ...
            JCHK(NULL != (pSourProfile = (IProfile*)pValSour->value),E_FAIL);
            JCHK(NULL != (pDestProfile = (IProfile*)pValDest->value),E_FAIL);
            if(0 != Compare(pDestProfile,pSourProfile))
                return COMPARE_DIFFERENT_VALUE;
        }
        else
        {
