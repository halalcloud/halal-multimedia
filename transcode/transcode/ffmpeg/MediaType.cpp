...
{
    return m_spProfile.Create(CLSID_CMemProfile,this);
}
...
STDMETHODIMP CMediaType::SetMajor(MediaMajorType major)
{
    ...
    if(NULL != m_pDesp)
    {
        JCHK(major == (MediaMajorType)m_pDesp->type,E_INVALIDARG);
    }
    m_major = major;
    return S_OK;
}
...
{
    JCHK(NULL != (m_pDesp = GetDescriptor(pName)),E_INVALIDARG);
    m_major = (MediaMajorType)m_pDesp->type;
    return S_OK;
}
...
STDMETHODIMP CMediaType::Compare(IMediaType* pMT)
{
...
STDMETHODIMP CMediaType::GetStreamInfo(int64_t* start_time,int64_t* length,int64_t* bitrate,bool* global_header,uint8_t** extra_data,int* extra_size)
{
    ...
    if(NULL != start_time)
    {
        if(false == spProfile->Read(STREAM_START_TIME_KEY,start_time))
            hr = E_INVALIDARG;
    }
    if(NULL != length)
    {
...
STDMETHODIMP CMediaType::SetStreamInfo(int64_t* start_time,int64_t* length,int64_t* bitrate,bool* global_header,uint8_t* extra_data,int* extra_size)
{
    dom_ptr<IProfile> spProfile(this);
    if(NULL != start_time)
    {
    ...
    if(NULL != extra_data)
    {
        JCHK(NULL != extra_size && 0 < *extra_size,E_INVALIDARG);
        JCHK(spProfile->Write(STREAM_EXTRADATA_KEY,extra_data,(size_t)*extra_size),E_FAIL);
    }
    else
    {
...
STDMETHODIMP CMediaType::GetVideoInfo(VideoMediaType* pix_fmt,int* width,int* height,int* ratioX,int* ratioY,int64_t* duration)
{
    ...
    if(NULL != duration)
    {
        ...
        {
            pVal = spProfile->Read(VIDEO_DURATION_KEY);
            if(NULL != pVal)
            {
                if(true == STR_CMP(pVal->type,typeid(int).name()))
                ...
                {
                    char* pUnit = NULL;
                    int64_t val = (int64_t)strtol((const char*)pVal->value, &pUnit, 10);
                    if(0 < val)
                    {
                        if(0 == strcmp(pUnit,"ms") || 0 == strcmp(pUnit,"MS"))
                            *duration = val * 10000;
                        else if(*pUnit == 's' || *pUnit == 'S')
                            *duration = val * 10000000;
                        else if(*pUnit == 'm' || *pUnit == 'M')
                            *duration = val * 600000000;
                        else if(*pUnit == 'h' || *pUnit == 'H')
                            *duration = val * 36000000000;
                        else
...
STDMETHODIMP CMediaType::SetVideoInfo(VideoMediaType* pix_fmt,int* width,int* height,int* ratioX,int* ratioY,int64_t* duration)
{
    ...
    if(NULL != duration)
    {
        JCHK(spProfile->Write(VIDEO_DURATION_KEY,duration),E_FAIL);
        spProfile->Erase(VIDEO_FPS_KEY);
    }
    return hr;
}
...
STDMETHODIMP CMediaType::FrameAlloc(IMediaFrame* frm)
{
    ...
    if(MST_RAWVIDEO == sub)
    {
        ...
        JIF(frm->Alloc((uint32_t)szBuf));
        JIF(frm->SetBuf(NULL,(uint32_t)szBuf));
...
STDMETHODIMP CMediaType::FrameToArray(uint8_t** dst_data,int* dst_linesize,IMediaFrame* src)
{
    ...
    if(MST_RAWVIDEO == sub)
    {
        ...
        JCHK(VMT_NONE < m_pix_fmt && VMT_NB > m_pix_fmt && 0 < m_width && 0 < m_height,E_FAIL);
        JCHK(0 < av_image_fill_arrays(dst_data,dst_linesize,src->GetBuf(),(AVPixelFormat)m_pix_fmt,src->info.stride,m_height,1),E_FAIL);
    }
    else if(MST_PCM == sub)
    {
        JCHK(0 < src->info.samples,E_INVALIDARG);
        JCHK(AMT_NONE < m_sample_fmt && AMT_NB > m_sample_fmt && 0 < m_channels && 0 < m_frame_size,E_FAIL);
		JCHK(0 < av_samples_fill_arrays(dst_data,dst_linesize,src->GetBuf(),m_channels,src->info.samples,(AVSampleFormat)m_sample_fmt,AUDIO_ALIGN),E_FAIL);
    }
    else
    {
        JCHK(false,E_FAIL);
...
{
    return MST_PCM == type ? &pcm_descriptor : avcodec_descriptor_get((AVCodecID)type);
}

const AVCodecDescriptor* CMediaType::GetDescriptor(const char* pName)
{
    return 0 == strcmp(pName,pcm_descriptor.name) ? &pcm_descriptor : avcodec_descriptor_get_by_name(pName);
}
...
HRESULT CMediaType::Compare(IMediaType* pDest,IMediaType* pSour)
{
    JCHK(NULL != pDest,E_INVALIDARG);
    JCHK(NULL != pSour,E_INVALIDARG);
    if(pDest == pSour)
        return S_OK;
    if(pDest->GetSub() == pSour->GetSub())
    {
        dom_ptr<IProfile> spDest,spSour;
        JCHK(spDest.p = static_cast<IProfile*>(pDest->Query(IID(IProfile))),E_FAIL);
        JCHK(spSour.p = static_cast<IProfile*>(pSour->Query(IID(IProfile))),E_FAIL);
        return spDest->Compare(spSour);
    }
    else
        return S_FALSE;
}
