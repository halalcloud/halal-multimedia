...
DOM_CLASS_EXPORT(CMediaType,"MediaType",MAKE_VERSION(0,0,0,0),NULL)
DOM_CLASS_EXPORT(CFFmpegDemuxer,FILTER_TYPE_SOURCE,MAKE_VERSION(0,0,0,0),(void*)CFFmpegDemuxer::UrlSupportQuery)
DOM_CLASS_EXPORT(CFFmpegMuxer,FILTER_TYPE_RENDER,MAKE_VERSION(0,0,0,0),(void*)CFFmpegMuxer::UrlSupportQuery)
DOM_CLASS_EXPORT(CFFmpegAudioDecoder,FILTER_TYPE_TRANSFORM,MAKE_VERSION(0,0,0,0),(void*)CFFmpegAudioDecoder::CheckMediaType)
DOM_CLASS_EXPORT(CFFmpegAudioEncoder,FILTER_TYPE_TRANSFORM,MAKE_VERSION(0,0,0,0),(void*)CFFmpegAudioEncoder::CheckMediaType)
DOM_CLASS_EXPORT(CFFmpegVideoDecoder,FILTER_TYPE_TRANSFORM,MAKE_VERSION(0,0,0,0),(void*)CFFmpegVideoDecoder::CheckMediaType)
DOM_CLASS_EXPORT(CFFmpegVideoEncoder,FILTER_TYPE_TRANSFORM,MAKE_VERSION(0,0,0,0),(void*)CFFmpegVideoEncoder::CheckMediaType)
DOM_CLASS_EXPORT(CFFmpegAudioResample,FILTER_TYPE_TRANSFORM,MAKE_VERSION(0,0,0,0),(void*)CFFmpegAudioResample::CheckMediaType)
DOM_CLASS_EXPORT(CFFmpegVideoScale,FILTER_TYPE_TRANSFORM,MAKE_VERSION(0,0,0,0),(void*)CFFmpegVideoScale::CheckMediaType)
DOM_CLASS_EXPORT(CFFmpegOSD,FILTER_TYPE_TRANSFORM,MAKE_VERSION(0,0,0,0),(void*)CFFmpegOSD::CheckMediaType)
...
HRESULT AVPacketToFrame(IMediaFrame* pFrame, AVPacket& packet,const AVRational& base,int frame_size)
{
    ...
    pFrame->info.dts = AV_NOPTS_VALUE == packet.dts ? MEDIA_FRAME_NONE_TIMESTAMP : int64_t(packet.dts * FRAME_TIMEBASE.den * av_q2d(base) + 0.5);
    pFrame->info.pts = AV_NOPTS_VALUE == packet.pts ? MEDIA_FRAME_NONE_TIMESTAMP : int64_t(packet.pts * FRAME_TIMEBASE.den * av_q2d(base) + 0.5);
    ...
    JIF(pFrame->Alloc(packet.size));
    JIF(pFrame->SetBuf(packet.data,packet.size));
    return hr;
}
...
HRESULT FrameToAVPacket(AVPacket* pPacket,IMediaFrame* pFrame,const AVRational* base)
{
	...
	pPacket->data = (uint8_t*)pFrame->GetBuf((uint32_t*)&pPacket->size);
	return S_OK;
}
