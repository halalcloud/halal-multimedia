        ...
        if(AV_CODEC_ID_H264 == m_pStream->codec->codec_id || AV_CODEC_ID_HEVC == m_pStream->codec->codec_id)
        {
            if(false == m_isGlobalHeader)
            {
                if(NULL == m_pStream->codec->extradata || 0 == m_pStream->codec->extradata_size)
                {
                    ...
                    uint8_t* pBuf;
                    uint32_t szBuf = 0;
                    JCHK(NULL != (pBuf = spFrame->GetBuf(&szBuf)) && 0 < szBuf,E_FAIL);
...
HRESULT CFFmpegMuxer::CStream::Push(IMediaFrame* pFrame)
{
    if(NULL == pFrame)
    {
        ...
        if(S_STREAM_EOF == (hr = m_pMuxer->m_pFG->Notify(IFilterEvent::Process,hr,m_pMuxer,m_spPin)))
            m_isEnd = true;
        else
            return hr;
    }
    else
    {
        ...
        if(0 != (pFrame->info.flag&MEDIA_FRAME_FLAG_SYNCPOINT))
        {
            if(m_frames.size() >= 75)
            {
                LOG(0,"ffmpeg muxer %s stream drop %d frames data",m_spMT->GetSubName(),m_frames.size());
                m_frames.clear();
            }
        }
...
{
    if(m_frames.empty())
        return true == m_isEnd ? S_STREAM_EOF : S_FALSE;
    else
...
{
    return true == m_isLive ? FILTER_FLAG_LIVE : 0;
}
...
STDMETHODIMP CFFmpegMuxer::Open()
{
    HRESULT hr = S_OK;
    ...
    GetOption(m_ctxFormat->priv_data,spProfile);
    if(0 == strcmp(m_ctxFormat->oformat->name,"hls"))
    {
        m_isLive = true;
        int hls_list_size = 0;
        if(true == spProfile->Read("hls_list_size",hls_list_size))
        {
            if(0 == hls_list_size)
                m_isLive = false;
        }
    ...
    m_isOpen = true;
    if(NULL != m_pFG)
        m_pFG->Notify(IFilterEvent::Open,hr,this,NULL,NULL);
    return hr;
}

STDMETHODIMP CFFmpegMuxer::Close()
{
    HRESULT hr = S_OK;
    if(false == m_isFirst)
    {
    ...
    if(true == m_isOpen)
    {
        ...
        if(NULL != m_pFG)
            hr = m_pFG->Notify(IFilterEvent::Close,hr,this,NULL,NULL);
        m_isOpen = false;
    }
    return hr;
}

STDMETHODIMP CFFmpegMuxer::SetTag(void* pTag)
{
    m_pTag = pTag;
    return S_OK;
}
...
STDMETHODIMP CFFmpegMuxer::OnGetMediaType(IInputPin* pPin,IMediaType* pMT)
{
...
STDMETHODIMP CFFmpegMuxer::OnGetMediaType(IOutputPin* pPin,IMediaType* pMT)
{
    return E_FAIL;
}

STDMETHODIMP CFFmpegMuxer::OnSetMediaType(IInputPin* pPin,IMediaType* pMT)
{
    HRESULT hr = S_OK;
...
STDMETHODIMP CFFmpegMuxer::OnSetMediaType(IOutputPin* pPin,IMediaType* pMT)
{
    return S_OK;
}

STDMETHODIMP CFFmpegMuxer::OnNotify(IFilterEvent::EventType type,HRESULT hr,IInputPin* pPinIn,IOutputPin* pPinOut,IMediaFrame* pFrame)
{
    if(type == IFilterEvent::Process)
        return S_OK;
    return m_pFG->Notify(type,hr,this,pPinIn,pPinOut,pFrame);
}

STDMETHODIMP CFFmpegMuxer::Load(const char* pUrl)
{
    HRESULT hr = S_OK;
    ...
    {
        if(0 == strcmp(url.protocol,"rtmp"))
        {
            pFormat = "flv";
            m_isLive = true;
        }
        else if(0 == strcmp(url.protocol,"http"))
        {
            if(NULL != url.format && 0 == strcmp(url.format,"ts"))
            {
                pFormat = "mpegts";
                m_isLive = true;
            }
    ...
    {
        if(NULL != url.format && 0 == strcmp(url.format,"m3u8"))
        {
            m_isLive = true;
        }
    }
...
STDMETHODIMP_(IInputPin*) CFFmpegMuxer::CreatePin(IMediaType* pMT)
{
    JCHK(NULL != pMT,NULL);
    ...
    JCHK(pStream->m_spPin.Create(CLSID_CInputPin,(IFilter*)this,&index),NULL);
...
STDMETHODIMP_(void) CFFmpegMuxer::Clear()
{
    Close();
    StreamIt it;
...
STDMETHODIMP CFFmpegMuxer::OnWriteFrame(IInputPin* pPin,IMediaFrame* pFrame)
{
    ...
    return WriteFrame(m_streams[index],pFrame);
...
HRESULT CFFmpegMuxer::WriteFrame(CStream* pStream,IMediaFrame* pFrame)
{
    ...
    JIF(m_pFG->Notify(IFilterEvent::Process,S_OK,this,pStream->m_spPin,NULL,pFrame));
...
HRESULT CFFmpegMuxer::UrlSupportQuery(const char* pProtocol,const char* pFormat)
{
    return 0;
}
